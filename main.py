...
def main():
    ...
    if dataMatrix.shape[1] > maxBlockToTrain:
        dataMatrix = dataMatrix[:, :maxBlockToTrain]  # размерность [n,N]

    # достаем значение DC из исходного сигнала
    mean = np.sum(dataMatrix, 0) / dataMatrix.shape[0]
    dataMatrix = dataMatrix - np.tile(mean, [dataMatrix.shape[0], 1])
    # Делаем объект k-svd для разряженного кодирования
    ...
    # Удаляем шум с картиночки с помощью итогового словаря
    ...
    cv2.imwrite("noisy.png", noisy.astype(np.uint8))
