...
class ksvd(object):
    ...
    def __init__(self, words, iteration, stopFlag='solidError', errGoal=10, atomNumber=20, preserveDC=False):
        ...
        self.stopFlag = stopFlag  # Этот параметр определяет, как сходятся периоды разреженного кодирования
    ...
    def OMP(self, dictionary, data, errorGoal, showFlag=True):
        """
        построение разреженного представления с использованием алгоритма Orthogonal Matching Pursuit
        :param dictionary: словарь с размерностью [n,K]
        :param data: картиночки с размерностью [n,N]
        :param errorGoal:
        :return:
        """
        n, N = data.shape
    ...
    def updateDictionary(self, data, dictionary, wordToUpdate, coefMatrix, ):
        """
        Обнавляем частичку словаря
        return:
        1) Обнавляем частичку словаря
        2) Делаем новую coefMatrix (тут все хорошо, потому что мы обновляем только не нулевые)
        3) Добавляем новый кусочек, если старый не используется
        """
        nonZeroEntry = np.nonzero(coefMatrix[wordToUpdate, :])
        nonZeroEntry = nonZeroEntry[0]
        if len(nonZeroEntry) < 1:  # если мы не используем данные
            addVector = 1
